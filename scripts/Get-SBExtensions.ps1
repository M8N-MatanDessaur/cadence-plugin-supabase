<#
.SYNOPSIS
    Every Postgres extension: installed ones with version and schema, and the available ones.
.EXAMPLE
    ./scripts/Get-SBExtensions.ps1 -Installed
#>
[CmdletBinding()]
param(
    [switch]$Installed,
...
$all = @(Fail-IfError (Get-Api '/api/plugins/supabase/extensions')); if ($Installed) { $all = @($all | Where-Object { $_.installed }) }; Out-Json $all 4
