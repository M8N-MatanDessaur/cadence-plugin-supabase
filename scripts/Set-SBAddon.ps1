<#
.SYNOPSIS
    Changes an add-on (compute, pitr, custom_domain, ipv4...). It changes the bill, so the server answers with a token first; repeat with -ConfirmToken.
.EXAMPLE
    ./scripts/Set-SBAddon.ps1 -Type compute_instance -Variant ci_small
#>
[CmdletBinding()]
param(
...
$r = Send-Api 'PATCH' "/api/plugins/supabase/mgmt/addons?$(if ($ConfirmToken) { "confirm=$(Esc $ConfirmToken)" } else { 'soft=1' })" @{ addon_type = $Type; addon_variant = $Variant }
if ($r.confirmRequired) { Show-Confirm $r; return }
Fail-IfError $r | Out-Null
[pscustomobject]@{ ok = $true; addon = $Type; variant = $Variant } | ConvertTo-Json
