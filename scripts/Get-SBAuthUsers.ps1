<#
.SYNOPSIS
    Users from auth: id, email, provider, created, last sign in. -Query searches email.
.EXAMPLE
    ./scripts/Get-SBAuthUsers.ps1 -Query gmail -PerPage 100
#>
[CmdletBinding()]
param(
    [string]$Query = '',
...
Fail-IfError (Get-Api "/api/plugins/supabase/auth/users?page=$Page&per_page=$PerPage$(if ($Query) { "&q=$(Esc $Query)" })") | ConvertTo-Json -Depth 6
