<#
.SYNOPSIS
    Reads the logs with SQL over the log tables (postgres_logs, edge_logs, auth_logs, function_logs, storage_logs), newest first.
.EXAMPLE
    ./scripts/Get-SBLogs.ps1 -Source auth_logs -Limit 100
#>
[CmdletBinding()]
param(
    [ValidateSet('postgres_logs','edge_logs','auth_logs','function_logs','storage_logs','realtime_logs')][string]$Source = 'postgres_logs',
    [int]$Limit = 50,
    [string]$Sql = '',
    [string]$Project = ''
)
...
$q = if ($Sql) { $Sql } else { "select timestamp, event_message from $Source order by timestamp desc limit $Limit" }
$r = Post-Api "/api/plugins/supabase/logs" @{ sql = $q }
if ($r.error) { throw "Supabase: $(ConvertTo-Json -InputObject $r.error -Compress)" }
$rows = if ($null -ne $r.result) { $r.result } else { $r }
Out-Json @($rows | ForEach-Object { if ($_.timestamp -and ($_.timestamp -as [long])) { $_.timestamp = [DateTimeOffset]::FromUnixTimeMilliseconds([long]($_.timestamp / 1000)).ToString('o') }; $_ }) 6
