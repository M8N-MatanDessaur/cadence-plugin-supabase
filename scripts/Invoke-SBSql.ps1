<#
.SYNOPSIS
    Runs SQL as postgres. Reads go through the read-only route; writes go through the gated route; destructive statements (DROP, TRUNCATE, DELETE without WHERE...) first answer with a token, and run when the same command is repeated with -ConfirmToken. -File reads the SQL from a file.
.EXAMPLE
    ./scripts/Invoke-SBSql.ps1 -Query 'select count(*) from profiles'
#>
[CmdletBinding()]
param(
    [string]$Query = '',
...
if ($File) { $Query = Read-Text $File }
if (-not $Query.Trim()) { throw 'Give -Query or -File' }
if ($ReadOnly) { $r = Post-Api "/api/plugins/supabase/sql/select" @{ query = $Query } }
else { $r = Post-Api "/api/plugins/supabase/sql?$(if ($ConfirmToken) { "confirm=$(Esc $ConfirmToken)" } else { 'soft=1' })" @{ query = $Query; readOnly = $false } }
if ($r.confirmRequired) { Show-Confirm $r; return }
Fail-IfError $r | Out-Null
if ($r -is [array] -or $r -is [System.Collections.IEnumerable] -and -not ($r -is [string])) { Out-Json @($r) 8 } else { Out-Json $r 8 }
