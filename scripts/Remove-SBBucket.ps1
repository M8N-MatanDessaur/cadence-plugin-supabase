<#
.SYNOPSIS
    Deletes a bucket. It must be empty; -Empty empties it first, which the server confirms with a token (repeat with -ConfirmToken).
.EXAMPLE
    ./scripts/Remove-SBBucket.ps1 -Name uploads -Empty
#>
[CmdletBinding()]
param(
...
if ($Empty) { $e = Post-Api "/api/plugins/supabase/storage/buckets/$(Esc $Name)/empty?$(if ($ConfirmToken) { "confirm=$(Esc $ConfirmToken)" } else { 'soft=1' })" @{}; if ($e.confirmRequired) { if (-not $e.warning) { $e | Add-Member -NotePropertyName warning -NotePropertyValue "empty the bucket $Name (every file deleted)" -Force }; Show-Confirm $e; return }; Fail-IfError $e | Out-Null }
$r = Send-Api 'DELETE' "/api/plugins/supabase/storage/buckets/$(Esc $Name)" $null
if ($r.message -and $r.statusCode -and $r.statusCode -ne '200') { throw "Supabase: $($r.message)" }
[pscustomobject]@{ ok = $true; deleted = $Name } | ConvertTo-Json
