<#
.SYNOPSIS
    Sets an edge function secret (or removes it with -Remove).
.EXAMPLE
    ./scripts/Set-SBSecret.ps1 -Name STRIPE_KEY -Value sk_live_...
#>
[CmdletBinding()]
param(
...
if ($Remove) { Fail-IfError (Send-Api 'DELETE' "/api/plugins/supabase/secrets" @($Name)) | Out-Null }
else { if (-not $Value) { throw 'Give -Value, or -Remove' }; Fail-IfError (Post-Api "/api/plugins/supabase/secrets" @(@{ name = $Name; value = $Value })) | Out-Null }
[pscustomobject]@{ ok = $true; secret = $Name; removed = [bool]$Remove } | ConvertTo-Json
