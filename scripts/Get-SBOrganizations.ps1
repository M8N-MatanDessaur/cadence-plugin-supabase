<#
.SYNOPSIS
    The organizations the management token can see, with their projects.
.EXAMPLE
    ./scripts/Get-SBOrganizations.ps1
#>
[CmdletBinding()]
param(
...
$orgs = @(Fail-IfError (Get-Api '/api/plugins/supabase/mgmt/organizations'))
$projects = @(Fail-IfError (Get-Api '/api/plugins/supabase/mgmt/projects'))
Out-Json @($orgs | ForEach-Object { $id = $_.id; [pscustomobject]@{ id = $id; name = $_.name; projects = @($projects | Where-Object { $_.organization_id -eq $id } | Select-Object id, name, region, status) } }) 6
