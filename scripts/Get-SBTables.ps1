<#
.SYNOPSIS
    The tables of a schema: rows (estimated), size, RLS on or off, policy count, primary key.
.EXAMPLE
    ./scripts/Get-SBTables.ps1 -Schema public
#>
[CmdletBinding()]
param(
...
Out-Json @(Fail-IfError (Get-Api "/api/plugins/supabase/tables?schema=$(Esc $Schema)")) 4
