<#
.SYNOPSIS
    The TypeScript types of the schema, as the CLI generates them. -OutFile writes them to a file.
.EXAMPLE
    ./scripts/Get-SBTypes.ps1 -OutFile src/lib/database.types.ts
#>
[CmdletBinding()]
param(
...
$r = Fail-IfError (Get-Api "/api/plugins/supabase/types/typescript?schemas=$(Esc $Schemas)")
$text = if ($r.types) { $r.types } else { [string]$r }
if ($OutFile) { [System.IO.File]::WriteAllText((Join-Path (Get-Location) $OutFile), $text, (New-Object System.Text.UTF8Encoding $false)); "Wrote $($text.Length) characters to $OutFile" } else { $text }
