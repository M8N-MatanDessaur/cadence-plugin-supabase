<#
.SYNOPSIS
    The indexes of a schema with definition, size and scans; -Unused lists only those never scanned.
.EXAMPLE
    ./scripts/Get-SBIndexes.ps1 -Unused
#>
[CmdletBinding()]
param(
    [switch]$Unused,
...
if ($Unused) { Out-Json @(Fail-IfError (Get-Api '/api/plugins/supabase/stats/unused-indexes')) 4 } else { Out-Json @(Fail-IfError (Get-Api "/api/plugins/supabase/indexes?schema=$(Esc $Schema)")) 4 }
