<#
.SYNOPSIS
    Updates the rows a PostgREST filter matches with the fields of a JSON file. A filter is required so nothing updates the whole table by accident.
.EXAMPLE
    ./scripts/Update-SBRows.ps1 -Table profiles -Filter 'id=eq.42' -JsonFile .ai-workspace/patch.json
#>
[CmdletBinding()]
param(
...
$r = Fail-IfError (Send-Api 'PATCH' "/api/plugins/supabase/rows/$(Esc $Table)?$($qs -join '&')" $doc)
Out-Json @($r) 8
