<#
.SYNOPSIS
    Creates a bucket, private unless -Public, with an optional size limit in bytes and allowed MIME types.
.EXAMPLE
    ./scripts/New-SBBucket.ps1 -Name uploads -FileSizeLimit 5242880 -AllowedTypes image/png,image/jpeg
#>
[CmdletBinding()]
param(
...
$payload = @{ id = $Name; name = $Name; public = [bool]$Public }
if ($FileSizeLimit -gt 0) { $payload.file_size_limit = $FileSizeLimit }
if ($AllowedTypes.Count) { $payload.allowed_mime_types = $AllowedTypes }
$r = Fail-IfError (Post-Api "/api/plugins/supabase/storage/buckets" $payload)
if ($r.message -and -not $r.name) { throw "Supabase: $($r.message)" }
[pscustomobject]@{ ok = $true; bucket = $Name; public = [bool]$Public } | ConvertTo-Json
