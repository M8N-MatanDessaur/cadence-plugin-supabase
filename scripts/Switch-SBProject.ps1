<#
.SYNOPSIS
    Makes a project the active one (the default when a shell is not on a repository).
.EXAMPLE
    ./scripts/Switch-SBProject.ps1 -Name 'my app'
#>
[CmdletBinding()]
param(
...
Fail-IfError (Post-Api "/api/plugins/supabase/projects/active" @{ name = $Name }) | Out-Null
[pscustomobject]@{ ok = $true; active = $Name } | ConvertTo-Json
