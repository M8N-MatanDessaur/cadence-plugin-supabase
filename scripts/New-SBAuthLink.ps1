<#
.SYNOPSIS
    Generates a magic link, recovery link, invite or signup link for an email, without sending mail. Prints the action link.
.EXAMPLE
    ./scripts/New-SBAuthLink.ps1 -Email someone@example.com -Type magiclink
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory)][string]$Email,
    [ValidateSet('magiclink','recovery','invite','signup')][string]$Type = 'magiclink',
    [string]$Password = '',
    [string]$RedirectTo = '',
    [string]$Project = ''
)
...
$r = Fail-IfError (Post-Api "/api/plugins/supabase/auth/generate-link" $payload)
if ($r.msg -and -not $r.action_link) { throw "Supabase: $($r.msg)" }
[pscustomobject]@{ ok = [bool]$r.action_link; type = $Type; email = $Email; link = $r.action_link; expires_in = 3600 } | ConvertTo-Json
