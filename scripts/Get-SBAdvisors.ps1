<#
.SYNOPSIS
    The raw security and performance advisor lints, the same the Supabase dashboard shows.
.EXAMPLE
    ./scripts/Get-SBAdvisors.ps1 -Kind security
#>
[CmdletBinding()]
param(
    [ValidateSet('security','performance','both')][string]$Kind = 'both',
    [string]$Project = ''
)
...
$out = [ordered]@{}
if ($Kind -ne 'performance') { $out.security = Fail-IfError (Get-Api '/api/plugins/supabase/advisors/security') }
if ($Kind -ne 'security') { $out.performance = Fail-IfError (Get-Api '/api/plugins/supabase/advisors/performance') }
Out-Json $out 10
