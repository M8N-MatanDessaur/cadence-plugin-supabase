...
.EXAMPLE
    ./scripts/Remove-SBPolicy.ps1 -Table profiles -Name 'Users read their own row'
#>
[CmdletBinding()]
param(
...
Fail-IfError (Send-Api 'DELETE' "/api/plugins/supabase/policies/$(Esc $Schema)/$(Esc $Table)/$(Esc $Name)" $null) | Out-Null
[pscustomobject]@{ ok = $true; dropped = $Name; table = "$Schema.$Table" } | ConvertTo-Json
