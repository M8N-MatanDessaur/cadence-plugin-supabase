<#
.SYNOPSIS
    The database functions of a schema (name, arguments, returns, language) and the triggers.
.EXAMPLE
    ./scripts/Get-SBFunctions.ps1
#>
[CmdletBinding()]
param(
...
[pscustomobject]@{ functions = @(Fail-IfError (Get-Api "/api/plugins/supabase/functions?schema=$(Esc $Schema)")); triggers = @(Fail-IfError (Get-Api "/api/plugins/supabase/triggers?schema=$(Esc $Schema)")) } | ConvertTo-Json -Depth 6
