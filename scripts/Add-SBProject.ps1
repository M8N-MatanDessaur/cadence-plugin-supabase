<#
.SYNOPSIS
    Adds a Supabase project: name, ref, service role key (and anon key), optional repository so the screen follows the shell. Keys stay on this machine.
.EXAMPLE
    ./scripts/Add-SBProject.ps1 -Name 'my app' -ProjectRef abcdefghijklmnopqrst -ServiceRoleKey <key> -RepoPath C:\Code\my-app
#>
[CmdletBinding()]
param(
...
$payload = @{ name = $Name; projectRef = $ProjectRef; url = $(if ($Url) { $Url } else { "https://$ProjectRef.supabase.co" }); repoPath = $RepoPath }
if ($ServiceRoleKey) { $payload.serviceRoleKey = $ServiceRoleKey }
if ($AnonKey) { $payload.anonKey = $AnonKey }
Fail-IfError (Post-Api "/api/plugins/supabase/projects" $payload) | Out-Null
if ($Activate) { Post-Api "/api/plugins/supabase/projects/active" @{ name = $Name } | Out-Null }
[pscustomobject]@{ ok = $true; project = $Name; ref = $ProjectRef; active = [bool]$Activate } | ConvertTo-Json
