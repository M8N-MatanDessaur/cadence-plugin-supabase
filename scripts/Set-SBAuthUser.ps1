<#
.SYNOPSIS
    Changes a user: -Ban (with a duration like 24h, or none to lift it), -ConfirmEmail, -Password, -SignOut, or fields from a JSON file (email, phone, user_metadata, app_metadata, role).
.EXAMPLE
    ./scripts/Set-SBAuthUser.ps1 -Id <uuid> -Ban 24h
#>
[CmdletBinding()]
param(
...
if ($SignOut) { Fail-IfError (Post-Api "/api/plugins/supabase/auth/users/$(Esc $Id)/sign-out" @{}) | Out-Null; [pscustomobject]@{ ok = $true; id = $Id; signedOut = $true } | ConvertTo-Json; return }
$payload = if ($JsonFile) { Read-JsonFile $JsonFile } else { [pscustomobject]@{} }
if ($Ban) { $payload | Add-Member -NotePropertyName ban_duration -NotePropertyValue $Ban -Force }
if ($ConfirmEmail) { $payload | Add-Member -NotePropertyName email_confirm -NotePropertyValue $true -Force }
if ($Password) { $payload | Add-Member -NotePropertyName password -NotePropertyValue $Password -Force }
$r = Fail-IfError (Send-Api 'PUT' "/api/plugins/supabase/auth/users/$(Esc $Id)" $payload)
if ($r.msg -and -not $r.id) { throw "Supabase: $($r.msg)" }
[pscustomobject]@{ ok = [bool]$r.id; id = $r.id; email = $r.email; banned_until = $r.banned_until; confirmed = [bool]$r.email_confirmed_at } | ConvertTo-Json
