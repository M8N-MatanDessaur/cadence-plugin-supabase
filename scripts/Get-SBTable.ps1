<#
.SYNOPSIS
    One table in full: columns with types, defaults and references, indexes, policies, triggers, constraints, the tables that reference it, and its statistics.
.EXAMPLE
    ./scripts/Get-SBTable.ps1 -Table profiles
#>
[CmdletBinding()]
param(
...
Fail-IfError (Get-Api "/api/plugins/supabase/table?schema=$(Esc $Schema)&name=$(Esc $Table)") | ConvertTo-Json -Depth 8
