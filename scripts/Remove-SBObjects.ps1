<#
.SYNOPSIS
    Deletes files from a bucket by path (several allowed). The server asks for approval.
.EXAMPLE
    ./scripts/Remove-SBObjects.ps1 -Bucket avatars -Path 'users/42.png','users/43.png'
#>
[CmdletBinding()]
param(
...
$r = Fail-IfError (Send-Api 'DELETE' "/api/plugins/supabase/storage/objects/$(Esc $Bucket)" @{ prefixes = $Path })
[pscustomobject]@{ ok = $true; bucket = $Bucket; deleted = @($r).Count; paths = $Path } | ConvertTo-Json
