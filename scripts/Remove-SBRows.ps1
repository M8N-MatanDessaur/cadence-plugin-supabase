<#
.SYNOPSIS
    Deletes the rows a PostgREST filter matches. A filter is required. Returns the deleted rows.
.EXAMPLE
    ./scripts/Remove-SBRows.ps1 -Table profiles -Filter 'id=eq.42'
#>
[CmdletBinding()]
param(
...
$r = Fail-IfError (Send-Api 'DELETE' "/api/plugins/supabase/rows/$(Esc $Table)?$($qs -join '&')" $null)
Out-Json @($r) 8
