<#
.SYNOPSIS
    Auth in numbers: totals, confirmed, growth over the last days, providers, MFA, sessions.
.EXAMPLE
    ./scripts/Get-SBUserStats.ps1 -Days 30
#>
[CmdletBinding()]
param(
    [int]$Days = 30,
...
[pscustomobject]@{
  summary   = Fail-IfError (Get-Api '/api/plugins/supabase/stats/auth/summary')
  growth    = @(Fail-IfError (Get-Api "/api/plugins/supabase/stats/auth/growth?days=$Days"))
  providers = @(Fail-IfError (Get-Api '/api/plugins/supabase/stats/auth/providers'))
  mfa       = Fail-IfError (Get-Api '/api/plugins/supabase/stats/auth/mfa')
  sessions  = Fail-IfError (Get-Api '/api/plugins/supabase/stats/auth/sessions')
