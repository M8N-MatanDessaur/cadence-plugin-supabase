<#
.SYNOPSIS
    A signed URL to download one private file, valid for -ExpiresIn seconds (one hour by default).
.EXAMPLE
    ./scripts/New-SBSignedUrl.ps1 -Bucket avatars -Path 'users/42.png'
#>
[CmdletBinding()]
param(
...
$r = Fail-IfError (Post-Api "/api/plugins/supabase/storage/sign/$(Esc $Bucket)/$(($Path -split '/' | ForEach-Object { Esc $_ }) -join '/')" @{ expiresIn = $ExpiresIn })
if ($r.message -and -not $r.signedURL) { throw "Supabase: $($r.message)" }
$cfg = Get-Api '/api/plugins/supabase/config'
[pscustomobject]@{ ok = [bool]$r.signedURL; url = "$($cfg.active.url)/storage/v1$($r.signedURL)"; expires_in = $ExpiresIn } | ConvertTo-Json
