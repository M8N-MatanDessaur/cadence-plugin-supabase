<#
.SYNOPSIS
    Database statistics: size and connections, table sizes, cache hit, locks, blocking, vacuum, long-running, slow queries (when pg_stat_statements is on).
.EXAMPLE
    ./scripts/Get-SBDbStats.ps1 -Kind slow-queries
#>
[CmdletBinding()]
param(
    [ValidateSet('overview','db','tables','cache-hit','locks','blocking','vacuum','long-running','slow-queries','unused-indexes','roles','replication-slots')][string]$Kind = 'overview',
    [string]$Project = ''
)
...
Fail-IfError (Get-Api "/api/plugins/supabase/stats/$Kind") | ConvertTo-Json -Depth 8
