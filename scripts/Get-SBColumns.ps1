<#
.SYNOPSIS
    The columns of a table: name, type, nullable, default, primary key, references.
.EXAMPLE
    ./scripts/Get-SBColumns.ps1 -Table profiles
#>
[CmdletBinding()]
param(
...
Out-Json @((Fail-IfError (Get-Api "/api/plugins/supabase/table?schema=$(Esc $Schema)&name=$(Esc $Table)")).columns) 4
