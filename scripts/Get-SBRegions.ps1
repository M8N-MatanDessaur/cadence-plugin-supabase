<#
.SYNOPSIS
    The regions a new project can be created in, for an organization (the first one by default).
.EXAMPLE
    ./scripts/Get-SBRegions.ps1
#>
[CmdletBinding()]
param(
    [string]$Org = ''
...
Out-Json (Fail-IfError (Get-Api "/api/plugins/supabase/mgmt/regions$(if ($Org) { "?org=$(Esc $Org)" })")) 4
