<#
.SYNOPSIS
    Inserts one row (or many) into a table from a JSON file: an object or an array of objects. Returns the inserted rows.
.EXAMPLE
    ./scripts/New-SBRow.ps1 -Table profiles -JsonFile .ai-workspace/row.json
#>
[CmdletBinding()]
param(
...
$r = Fail-IfError (Post-Api "/api/plugins/supabase/rows/$(Esc $Table)?_schema=$(Esc $Schema)" $doc)
Out-Json @($r) 8
