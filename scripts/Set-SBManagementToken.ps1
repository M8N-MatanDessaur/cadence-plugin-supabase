<#
.SYNOPSIS
    Stores the account management token (sbp_...) that reaches every project.
.EXAMPLE
    ./scripts/Set-SBManagementToken.ps1 -Token sbp_...
#>
[CmdletBinding()]
param(
...
Fail-IfError (Post-Api "/api/plugins/supabase/config" @{ managementToken = $Token }) | Out-Null
[pscustomobject]@{ ok = $true; managementTokenSet = $true } | ConvertTo-Json
