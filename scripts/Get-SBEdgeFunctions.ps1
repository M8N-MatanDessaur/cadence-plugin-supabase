<#
.SYNOPSIS
    The edge functions: slug, status, version, JWT required or not, updated.
.EXAMPLE
    ./scripts/Get-SBEdgeFunctions.ps1
#>
[CmdletBinding()]
param(
    [string]$Project = ''
...
Out-Json @(Fail-IfError (Get-Api '/api/plugins/supabase/edge/functions') | Select-Object slug, name, status, version, verify_jwt, updated_at) 4
