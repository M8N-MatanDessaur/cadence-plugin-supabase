<#
.SYNOPSIS
    Turns row level security on for a table (or off with -Disable, which needs a confirmation on the server).
.EXAMPLE
    ./scripts/Enable-SBRls.ps1 -Table profiles
#>
[CmdletBinding()]
param(
...
$r = Fail-IfError (Post-Api "/api/plugins/supabase/rls/$verb/$(Esc $Schema)/$(Esc $Table)" @{})
[pscustomobject]@{ ok = $true; table = "$Schema.$Table"; rls = -not $Disable } | ConvertTo-Json
