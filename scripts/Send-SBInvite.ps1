<#
.SYNOPSIS
    Invites a user by email: Supabase sends the invite mail.
.EXAMPLE
    ./scripts/Send-SBInvite.ps1 -Email someone@example.com
#>
[CmdletBinding()]
param(
...
$r = Fail-IfError (Post-Api "/api/plugins/supabase/auth/invite" @{ email = $Email })
if ($r.msg -and -not $r.id) { throw "Supabase: $($r.msg)" }
[pscustomobject]@{ ok = [bool]$r.id; id = $r.id; email = $r.email; invited_at = $r.invited_at } | ConvertTo-Json
