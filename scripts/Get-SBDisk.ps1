<#
.SYNOPSIS
    Disk: size, used, free, type and IOPS.
.EXAMPLE
    ./scripts/Get-SBDisk.ps1
#>
[CmdletBinding()]
param(
    [string]$Project = ''
...
$d = Get-Api '/api/plugins/supabase/mgmt/disk'
$u = Get-Api '/api/plugins/supabase/mgmt/disk/util'
[pscustomobject]@{ type = $d.type; size_gb = $d.size_gb; iops = $d.iops; throughput_mbps = $d.throughput_mbps; used_bytes = $u.metrics.fs_used_bytes; size_bytes = $u.metrics.fs_size_bytes; free_bytes = $u.metrics.fs_avail_bytes; read_at = $u.timestamp } | ConvertTo-Json
