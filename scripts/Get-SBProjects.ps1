<#
.SYNOPSIS
    The Supabase projects this workspace knows: name, ref, URL, which keys are set, repository, and which is active.
.EXAMPLE
    ./scripts/Get-SBProjects.ps1
#>
[CmdletBinding()]
param(
...
$c = Get-Api '/api/plugins/supabase/config'
$p = @((Get-Api '/api/plugins/supabase/projects').projects)
[pscustomobject]@{ managementTokenSet = $c.managementTokenSet; active = $c.active.name; projects = @($p | Select-Object name, projectRef, url, serviceRoleKeySet, anonKeySet, repoPath) } | ConvertTo-Json -Depth 4
