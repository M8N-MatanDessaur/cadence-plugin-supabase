<#
.SYNOPSIS
    Rows of a table through PostgREST: -Select columns, -Filter as PostgREST pairs (e.g. status=eq.active), -Order, -Limit, -Offset. Prints rows and the total.
.EXAMPLE
    ./scripts/Get-SBRows.ps1 -Table profiles -Filter 'plan=eq.pro' -Order 'created_at.desc' -Limit 20
#>
[CmdletBinding()]
param(
...
$qs = @("select=$(Esc $Select)", "limit=$Limit", "offset=$Offset", "_schema=$(Esc $Schema)", '_meta=1')
if ($Order) { $qs += "order=$(Esc $Order)" }
foreach ($f in $Filter) { $i = $f.IndexOf('='); if ($i -lt 1) { throw "Filter must be column=op.value: $f" }; $qs += "$(Esc $f.Substring(0, $i))=$(Esc $f.Substring($i + 1))" }
$r = Get-Api "/api/plugins/supabase/rows/$(Esc $Table)?$($qs -join '&')"
if ($r.error) { throw "Supabase: $($r.error)" }
Out-Json $r 8
