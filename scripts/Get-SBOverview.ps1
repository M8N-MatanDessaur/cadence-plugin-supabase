<#
.SYNOPSIS
    The project at a glance: services, database, users, storage, the tables with rows, size, RLS and policies, buckets, functions, advisor counts, disk, backups, migrations, the newest users, and the issues worth fixing first.
.EXAMPLE
    ./scripts/Get-SBOverview.ps1
#>
[CmdletBinding()]
param(
    [switch]$Refresh,
...
Fail-IfError (Get-Api "/api/plugins/supabase/overview$(if ($Refresh) { '?refresh=1' })") | ConvertTo-Json -Depth 8
