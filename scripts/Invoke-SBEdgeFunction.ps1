<#
.SYNOPSIS
    Calls an edge function with a JSON body (inline or from a file), using the service role.
.EXAMPLE
    ./scripts/Invoke-SBEdgeFunction.ps1 -Slug hello -Body '{"name":"world"}'
#>
[CmdletBinding()]
param(
...
$payload = if ($JsonFile) { Read-JsonFile $JsonFile } else { ConvertFrom-Json -InputObject $Body }
$r = Post-Api "/api/plugins/supabase/edge/invoke/$(Esc $Slug)" $payload
Out-Json $r 10
