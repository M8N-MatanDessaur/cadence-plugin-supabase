<#
.SYNOPSIS
    Restarts the project services (brief downtime). The server answers with a token first; repeat with -ConfirmToken.
.EXAMPLE
    ./scripts/Restart-SBServices.ps1
#>
[CmdletBinding()]
param(
    [string]$ConfirmToken = '',
...
$r = Post-Api "/api/plugins/supabase/mgmt/project/restart-services?$(if ($ConfirmToken) { "confirm=$(Esc $ConfirmToken)" } else { 'soft=1' })" @{}
if ($r.confirmRequired) { Show-Confirm $r; return }
Fail-IfError $r | Out-Null
[pscustomobject]@{ ok = $true; restarting = $true } | ConvertTo-Json
