<#
.SYNOPSIS
    The row level security policies of a schema (or one table): command, roles, using and check expressions.
.EXAMPLE
    ./scripts/Get-SBPolicies.ps1 -Table profiles
#>
[CmdletBinding()]
param(
    [string]$Table = '',
...
Out-Json @(Fail-IfError (Get-Api "/api/plugins/supabase/policies?schema=$(Esc $Schema)$(if ($Table) { "&table=$(Esc $Table)" })")) 4
