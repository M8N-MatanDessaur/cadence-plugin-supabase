<#
.SYNOPSIS
    Enables a Postgres extension (or removes it with -Remove).
.EXAMPLE
    ./scripts/Set-SBExtension.ps1 -Name pg_stat_statements
#>
[CmdletBinding()]
param(
...
if ($Remove) { $r = Fail-IfError (Send-Api 'DELETE' "/api/plugins/supabase/extensions/$(Esc $Name)" $null) } else { $r = Fail-IfError (Post-Api "/api/plugins/supabase/extensions" @{ name = $Name }) }
[pscustomobject]@{ ok = $true; extension = $Name; removed = [bool]$Remove } | ConvertTo-Json
