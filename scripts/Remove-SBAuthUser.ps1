<#
.SYNOPSIS
    Deletes a user for good. The server answers with a token first; repeat with -ConfirmToken.
.EXAMPLE
    ./scripts/Remove-SBAuthUser.ps1 -Id <uuid>
#>
[CmdletBinding()]
param(
...
$r = Send-Api 'DELETE' "/api/plugins/supabase/auth/users/$(Esc $Id)?$(if ($ConfirmToken) { "confirm=$(Esc $ConfirmToken)" } else { 'soft=1' })" $null
if ($r.confirmRequired) { $r | Add-Member -NotePropertyName warning -NotePropertyValue "delete the user $Id for good" -Force; Show-Confirm $r; return }
if ($r.msg -and $r.code -ge 400) { throw "Supabase: $($r.msg)" }
[pscustomobject]@{ ok = $true; deleted = $Id } | ConvertTo-Json
