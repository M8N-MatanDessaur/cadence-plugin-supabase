<#
.SYNOPSIS
    The storage buckets with public or private, size limits, allowed types, and sizes.
.EXAMPLE
    ./scripts/Get-SBBuckets.ps1
#>
[CmdletBinding()]
param(
    [string]$Project = ''
...
$b = @(Fail-IfError (Get-Api '/api/plugins/supabase/storage/buckets'))
$sizes = @{}; try { foreach ($s in @(Get-Api '/api/plugins/supabase/stats/bucket-sizes')) { $sizes[$s.bucket_id] = $s } } catch {}
Out-Json @($b | ForEach-Object { $s = $sizes[$_.id]; [pscustomobject]@{ id = $_.id; name = $_.name; public = $_.public; file_size_limit = $_.file_size_limit; allowed_mime_types = $_.allowed_mime_types; objects = if ($s) { $s.objects } else { $null }; bytes = if ($s) { $s.bytes } else { $null }; created_at = $_.created_at } }) 4
