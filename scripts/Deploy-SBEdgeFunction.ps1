<#
.SYNOPSIS
    Deploys an edge function from a source file (TypeScript or JavaScript), creating it or bumping its version. -NoJwt makes it callable without a token.
.EXAMPLE
    ./scripts/Deploy-SBEdgeFunction.ps1 -Slug hello -File supabase/functions/hello/index.ts
#>
[CmdletBinding()]
param(
...
$src = Read-Text $File
$r = Fail-IfError (Post-Api "/api/plugins/supabase/edge/deploy" @{ slug = $Slug; name = $(if ($Name) { $Name } else { $Slug }); body = $src; verify_jwt = -not $NoJwt })
if ($r.message -and -not $r.slug) { throw "Supabase: $($r.message)" }
[pscustomobject]@{ ok = [bool]$r.slug; slug = $r.slug; version = $r.version; status = $r.status; verify_jwt = $r.verify_jwt } | ConvertTo-Json
