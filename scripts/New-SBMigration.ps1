<#
.SYNOPSIS
    Applies a migration: a name and the SQL from a file, recorded in the migration history.
.EXAMPLE
    ./scripts/New-SBMigration.ps1 -Name add_profiles_bio -File supabase/migrations/20260904_add_bio.sql
#>
[CmdletBinding()]
param(
...
$sql = Read-Text $File
$r = Fail-IfError (Post-Api "/api/plugins/supabase/migrations" @{ name = $Name; query = $sql })
if ($r.message -and -not $r.version) { throw "Supabase: $($r.message)" }
[pscustomobject]@{ ok = $true; name = $Name; version = $r.version } | ConvertTo-Json
