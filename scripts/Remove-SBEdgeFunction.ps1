<#
.SYNOPSIS
    Deletes an edge function (the server asks for approval).
.EXAMPLE
    ./scripts/Remove-SBEdgeFunction.ps1 -Slug hello
#>
[CmdletBinding()]
param(
...
Fail-IfError (Send-Api 'DELETE' "/api/plugins/supabase/edge/functions/$(Esc $Slug)" $null) | Out-Null
[pscustomobject]@{ ok = $true; deleted = $Slug } | ConvertTo-Json
