<#
.SYNOPSIS
    Creates a user with an email and password (confirmed unless -Unconfirmed), with optional user metadata from a JSON file.
.EXAMPLE
    ./scripts/New-SBAuthUser.ps1 -Email someone@example.com -Password 'a strong one'
#>
[CmdletBinding()]
param(
...
$payload = @{ email = $Email; password = $Password; email_confirm = -not $Unconfirmed }
if ($MetadataJsonFile) { $payload.user_metadata = Read-JsonFile $MetadataJsonFile }
$r = Fail-IfError (Post-Api "/api/plugins/supabase/auth/users" $payload)
if ($r.msg -and -not $r.id) { throw "Supabase: $($r.msg)" }
[pscustomobject]@{ ok = [bool]$r.id; id = $r.id; email = $r.email; confirmed = [bool]$r.email_confirmed_at } | ConvertTo-Json
