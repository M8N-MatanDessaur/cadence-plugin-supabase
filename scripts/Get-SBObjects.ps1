<#
.SYNOPSIS
    The files of a bucket under a prefix (folder): name, size, type, updated.
.EXAMPLE
    ./scripts/Get-SBObjects.ps1 -Bucket avatars -Prefix 'users/'
#>
[CmdletBinding()]
param(
...
$r = Fail-IfError (Post-Api "/api/plugins/supabase/storage/list/$(Esc $Bucket)" @{ prefix = $Prefix; limit = $Limit; offset = $Offset; sortBy = @{ column = 'name'; order = 'asc' } })
Out-Json @($r | ForEach-Object { [pscustomobject]@{ name = $_.name; folder = ($null -eq $_.id); size = $_.metadata.size; type = $_.metadata.mimetype; updated_at = $_.updated_at } }) 4
